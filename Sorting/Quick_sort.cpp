...
namespace detail
{
    ...
    template <typename T>
    void QuickSort (T *array, int begin, int end, bool(*Compare)(T,T))
    {
        ...
        QuickSort (array, begin, info.low, Compare);
        QuickSort (array, info.high, end, Compare);
    }
}
...
{
    if (size > 1)
        detail::QuickSort(array, 0, size-1, Compare);
}
...
{
    if (size > 1)
        detail::QuickSort(array, 0, size-1, (bool(*)(T,T))NULL);
}
